...
| **Clash / Mihomo**             | `/clash/`        | `subClashEnable` | Full Mihomo-compatible YAML profile.                |
| **Mihomo (explicit)**          | `/mihomo/`       | `subClashEnable` | Alias for the full `/clash/` profile.               |
...
Automatic Clash format detection no longer treats every User-Agent containing
the word `clash` as Mihomo. Its default matcher covers Mihomo-based clients and
excludes Clash for Windows; legacy clients must use the explicit legacy path.
