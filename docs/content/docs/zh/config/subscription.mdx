...
| **Clash / Mihomo**           | `/clash/`        | `subClashEnable` | 完整的 Mihomo 兼容 YAML 配置。                      |
| **Mihomo（明确端点）**       | `/mihomo/`       | `subClashEnable` | 完整 `/clash/` 配置的别名。                         |
...
Clash 格式自动识别也不再把 User-Agent 中所有包含 `clash` 的客户端都视为
Mihomo。默认匹配器只覆盖 Mihomo 系客户端并排除 Clash for Windows；旧版客户端必须使用明确的旧版路径。
